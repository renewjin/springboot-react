...
const SearchResult = () => {
    ...
    useEffect(() => {
        if(query) {
            ...
            .catch((error) => console.error("문제가 발생하여 검색을 가져오지 못합니다.", error))
        }
    }, [query]);
    ...
    return (
        <div className='chicken-list'>
            <h2>검색 결과 : "{query}"</h2>
            {chickens.length > 0 ? (
                chickens.map((chickne) => (
                    <div key={chickne.id} className='chicken-item'>
                        <h3>{chickne.chickenName}</h3>
                        <p>{chickne.descirption}</p>
                        <p>{chickne.price}원</p>
                    </div>
                ))
