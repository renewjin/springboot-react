...
public class PaymentController {
	...
	// payment 라는 주소로 결제 정보가 들어오면 결제 확인 창구로 넘겨주는 것 결제정보와 결제하고자 하는 사용자의 비밀번호가 담겨있음
	@PostMapping("/payment")
...
/*****
 ...
 ResponseEntity = 사용자가 요청한 응답을 개발자가 다시 사용자한데 전달할 때 사용
 
 
 HttpEntity : HTTP 요청 또는 응답의 본문(body)과 해더를 포함하는 객체
 HTTP 요청을 보낼 때 본문과 헤더를 설정하고자 할 때 사용
 본문(body) : 실제 전송될 데이터 ex) 아이디 비미리번호 작성한 글 등
 해더(headers) : HTTP 헤더 정보를 포함 ex) 글자인지 이미지인지 동영상인지 ... 어떤 파일이 들어오는 것인가? 누가보내나?
 HttpEntity<문자열이면 문자열, 숫자면 숫자, 모르겠으면 비워두기> abc = new HttpEntity<비워져있어도 상관없음>("요청 본문", headers);
 ...
 ResponseEntity<String이면String, Integer면 Integer 여러값이면 ?, 모르겠으면 비워두기> = new ResponseEntity<>("응답본문",headers);
 ...
 RequestEntity<String이면String, Integer면 Integer, 여러값이면 ?, 모르겠으면 비워우기> = new RequestEntity<>("요청본문", headers); 			
 RequestEntity<String> req = new RequestEntity<>("요청본문", headers, HttpMethod.POST, url);
  			
 차이점 요약하기
 	클래스					상속관계					주요사용목적						추가정보
 	HttpEntity				기본 클래스				Http 요청/응답 본문과 해더 포함		상태코드없음(성공여부)
 	ResponseEntity			HttpEntity상속			HTTP 응답 반환						상태코드포함(성공여부)
 	RequestEntity			HttpEntity상속			HTTP 요청 반환						URI와 HTTP 메서드 포함
